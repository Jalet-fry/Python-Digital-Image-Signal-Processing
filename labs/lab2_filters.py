...
def update_plots(label):
    ...
    if group == "clean":
        ...
        main_axes[0].plot(t_axis[:N_pts]*1000, x_clean[:N_pts], color="#2980b9", alpha=0.8, label='Чистый сигнал', linewidth=1.5)
        main_axes[0].plot(t_axis[:N_pts]*1000, total_noise[:N_pts], color="#7f8c8d", alpha=0.5, label='Шум (белый + 1500 Гц)', linewidth=1)
        main_axes[0].plot(t_axis[:N_pts]*1000, x_noisy[:N_pts], color="#e67e22", alpha=0.8, label='Зашумленный (чистый + шум)', linewidth=1.5)
        ...
        main_axes[0].text(0.95, 0.95, f'SNR = {snr_val:.2f} dB', transform=main_axes[0].transAxes, ha='right', va='top', bbox=dict(boxstyle='round', facecolor='white', alpha=0.8, edgecolor='#e67e22'))

        f_clean, m_clean = get_spectrum(x_clean)
        f_noise, m_noise = get_spectrum(total_noise)
        f_noisy, m_noisy = get_spectrum(x_noisy)
        main_axes[1].plot(f_clean, m_clean, color="#2980b9", linewidth=1.5, label='Чистый сигнал')
        main_axes[1].fill_between(f_noise, m_noise, color="#7f8c8d", alpha=0.3, label='Шум')
        main_axes[1].plot(f_noisy, m_noisy, color="#e67e22", linewidth=1.5, label='Зашумленный')
        for idx, freq_h in enumerate([110, 220, 330, 440]): main_axes[1].axvline(x=freq_h, color='red', linestyle='--', alpha=0.5, label=f'{freq_h} Гц' if idx == 0 else '')
        main_axes[1].axvline(x=1500, color='purple', linestyle='-', alpha=0.7, label='Наводка 1500 Гц', linewidth=2)
        main_axes[1].set_title("Сравнение спектров", fontweight='bold', fontsize=11); main_axes[1].set_xlim(0, 2500)
        main_axes[1].legend(loc='upper right', fontsize=7); main_axes[1].grid(True, alpha=0.3, linestyle='--')

        main_axes[2].hist(x_clean, bins=50, alpha=0.5, color='#2980b9', label='Чистый', edgecolor='black', linewidth=0.5)
        main_axes[2].hist(x_noisy, bins=50, alpha=0.5, color='#e67e22', label='Зашумленный', edgecolor='black', linewidth=0.5)
        main_axes[2].set_title("Сравнение распределений амплитуд", fontweight='bold', fontsize=11); main_axes[2].legend(loc='upper right', fontsize=7)

    elif group == "filter":
        sig, col, snr = data["sig"], data["col"], data["snr"]
        # Расчет АЧХ для графика
        if "MA" in label:
            b, a = np.ones(lab2_cfg['M_ma'])/lab2_cfg['M_ma'], [1.0]
            p_band = (0, sr/(2*lab2_cfg['M_ma']))
        elif "КИХ" in label:
            b, a = h_fir, [1.0]
            p_band = (f_low, f_high)
        else:
            b, a = b_iir, a_iir
            p_band = (lab2_cfg['iir']['f0']-lab2_cfg['iir']['bw']/2, lab2_cfg['iir']['f0']+lab2_cfg['iir']['bw']/2)

        w, h_resp = freqz(b, a, worN=2000, fs=sr)

        # 1. Временная область
        main_axes[0].plot(t_axis[:N_pts]*1000, x_noisy[:N_pts], color='orange', alpha=0.4, label='Вход (Зашумленный)')
        main_axes[0].plot(t_axis[:N_pts]*1000, sig[:N_pts], color=col, lw=1.5, label='Выход (Отфильтрованный)')
        main_axes[0].set_title(f"Результат фильтрации (Улучшение SNR до {snr:.2f} dB)", fontweight='bold'); main_axes[0].legend(fontsize=8)

        # 2. Спектральная очистка (наглядная)
        f_in, m_in = get_spectrum(x_noisy)
        f_out, m_out = get_spectrum(sig)
        main_axes[1].fill_between(f_in, m_in, color='orange', alpha=0.2, label='Спектр Входа')
        main_axes[1].fill_between(f_out, m_out, color=col, alpha=0.6, label='Спектр Выхода')
        main_axes[1].axvspan(p_band[0], p_band[1], color='green', alpha=0.1, label='Полоса пропускания')
        main_axes[1].set_title("Сравнение спектров ДО и ПОСЛЕ", fontweight='bold'); main_axes[1].set_xlim(0, 2500)
        main_axes[1].legend(loc='upper right', fontsize=8)

        # 3. АЧХ фильтра (Линейный масштаб - нагляднее для понимания)
        main_axes[2].plot(w, np.abs(h_resp), color='black', lw=2, label='АЧХ (линейная)')
        main_axes[2].fill_between(w, np.abs(h_resp), color=col, alpha=0.3)
        main_axes[2].axhline(y=0.707, color='red', linestyle='--', alpha=0.5, label='Уровень -3дБ (0.707)')
        main_axes[2].set_title(f"АЧХ спроектированного фильтра ({label})", fontweight='bold')
        ...
        main_axes[2].legend(loc='upper right', fontsize=8)
